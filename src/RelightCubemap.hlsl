...
float3 sampleScene(float2 offset, float2 posSS, float3 albedoDiffuse)
{
	uint2 samplePos = posSS + offset;
...
float4 PS(in VSOutput input) : SV_Target
{
	...
	float3 s1 = sampleScene(float2(-0.5, -0.5), input.Position.xy, albedo);
    float3 s2 = sampleScene(float2(0.5, -0.5), input.Position.xy, albedo);
    float3 s3 = sampleScene(float2(-0.5, 0.5), input.Position.xy, albedo);
    float3 s4 = sampleScene(float2(0.5, 0.5), input.Position.xy, albedo);
